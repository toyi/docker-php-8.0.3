...
RUN which ssh-agent || (apk add -qq openssh-client) && eval $(ssh-agent -s)
RUN docker-php-ext-install gd gettext phar soap zip
RUN php -r "copy('https://getcomposer.org/installer', 'composer-setup.php');"
RUN php composer-setup.php && php -r "unlink('composer-setup.php');" && mv composer.phar /usr/local/bin
...
RUN php -r "echo ini_get('memory_limit');";
